...
#                 df = pd.read_csv(file)
#             else:
#                 return Response({"status":False, "message": "Unsupported file format. Please upload an Excel or CSV file."}, status=status.HTTP_400_BAD_REQUEST)

#             # Ensure required columns exist in the DataFrame
#             required_columns = {"first_name", "last_name", "email", "whatsapp_number", "gender", "dob", "anniversary_date", "city"}
#             if not required_columns.issubset(set(df.columns)):
#                 return Response({"status":False, "message": f"Missing required columns. Required: {required_columns}"}, status=status.HTTP_400_BAD_REQUEST)
...
#             if not required_columns.issubset(set(df.columns)):
#                 return Response(
#                     {"status": False, "message": f"Missing required columns. Required: {required_columns}"},
#                     status=status.HTTP_400_BAD_REQUEST,
#                 )
...
#                 status=status.HTTP_201_CREATED,
#             )
#         return Response({"status": False, "message": str(str(serializer.errors))}, status=status.HTTP_400_BAD_REQUEST)
